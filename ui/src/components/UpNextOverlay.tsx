...
export default function UpNextOverlay({ video, autoplayEnabled, onToggleAutoplay, onPlayNow, onDismiss }: {
  video: Video;
  ...
  }, [video.video_id, autoplayEnabled]);

  const progress = 1 - Math.max(0, remaining) / COUNTDOWN_SECONDS;

  return (
    <div className="up-next-overlay">
      <div className="up-next-bg" style={{ backgroundImage: `url(${img(video.thumbnail)})` }} aria-hidden="true" />
