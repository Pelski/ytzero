...
export default function DownloadAutomation() {
  ...
  return (
    <div className="dl-automation">
      ...
      {!canManage && <Alert variant="info">{tx("Only an administrator can change shared download rules.", "Tylko administrator może zmieniać wspólne reguły pobierania.", "Nur Administratoren können gemeinsame Download-Regeln ändern.")}</Alert>}
      {error && <Alert variant="danger">{error}</Alert>}
