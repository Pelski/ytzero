...
if (phase === "seed") {
  ...
  const qualities = db.prepare("SELECT user_id,value FROM plugin_settings WHERE plugin_id='downloads' AND key='quality' ORDER BY user_id").all();
  const retentions = db.prepare("SELECT user_id,value FROM plugin_settings WHERE plugin_id='downloads' AND key='retention_days' ORDER BY user_id").all();
  const outputTemplate = db.prepare("SELECT value FROM settings WHERE key='plugin_downloads_output_template'").get();
  ...
  console.log("RESULT " + JSON.stringify({ owner, jobOwner, ruleOwner, qualities, retentions, outputTemplate, cookiesConfigured, legacyCookiesRemain }));
  db.prepare("DELETE FROM download_owners WHERE video_id='legacy-download'").run();
