...
const secondaryQuality = await body(secondary.id, "/downloads/config", { method: "PUT", body: JSON.stringify({ settings: { quality: "720", retention_days: 30 } }) });
...
db.prepare("INSERT INTO plugin_settings(plugin_id,user_id,key,value) VALUES('downloads',1,'retention_days','14') ON CONFLICT(plugin_id,user_id,key) DO UPDATE SET value='14'").run();
db.prepare("INSERT INTO plugin_settings(plugin_id,user_id,key,value) VALUES('downloads',?,'retention_days','60') ON CONFLICT(plugin_id,user_id,key) DO UPDATE SET value='60'").run(secondary.id);
