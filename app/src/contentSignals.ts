...
export function recordSchedulingSignal(userId: number, videoId: string, bucket: string): boolean {
  ...
  insertSchedulingEvent.run(userId, videoId, video.channel_id, bucket, JSON.stringify(tagsForVideo(userId, videoId)), local.day, local.hour);
  return true;
}

/** Attribute a real playback delta to every effective tag active at the time. */
export function recordWatchTagSignals(
  userId: number,
  videoId: string,
  seconds: number,
  local = zonedDayHour(),
  ...
  db.transaction((items: EffectiveTagSnapshot[]) => {
    for (const tag of items) upsertWatchTagTime.run(userId, tag.id, tag.name, tag.color, local.day, local.hour, seconds);
  })(tags);
  return tags.length;
}
