...
describe("manual channel refresh schedules", () => {
  const schedule = { days: [1, 3], times: ["08:02", "18:02"] }; // Monday and Wednesday

  test("parses and validates stored values", () => {
    expect(parseManualRefreshSchedule("[3,1,3]", '["18:02","08:02","18:02"]')).toEqual(schedule);
    expect(parseManualRefreshSchedule("[3,1]", "18:02")).toEqual({ days: [1, 3], times: ["18:02"] });
  ...
  test("finds occurrences in the configured timezone", () => {
    ...
    expect(new Date(nextScheduleOccurrenceMs(schedule, now, "Europe/Warsaw")).toISOString()).toBe("2026-07-29T06:02:00.000Z");
  });

  test("becomes due once per scheduled occurrence", () => {
    const now = Date.parse("2026-07-27T16:03:00Z");
    expect(manualScheduleIsDue(schedule, "2026-07-27 06:02:05", now, "Europe/Warsaw")).toBe(true);
