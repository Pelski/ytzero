...
export const BACKUP_SECTIONS: readonly BackupSectionDefinition[] = [
  ...
  { id: "profile.followed-playlists", schemaVersion: 2, scope: "profile", sensitivity: "normal", dependencies: ["profiles.index", "instance.channels"], category: "organization", path: profilePath("followed-playlists.jsonl") },
  ...
  try {
    ...
    const tx = database.transaction(async () => {
      ...
      for (const profile of state.manifest.profiles) {
        ...
        if (selected.has("profile.followed-playlists")) { if (state.plan!.strategy === "replace") await database.prepare("DELETE FROM user_followed_playlists WHERE user_id=?").run(uid); for (const row of get("profile.followed-playlists") ?? []) { await ensureChannel({ channel_id: row.channel_id }); await database.prepare("INSERT INTO channel_playlists(playlist_id,channel_id,title,thumbnail,video_count) VALUES(?,?,?,?,?) ON CONFLICT(playlist_id) DO UPDATE SET title=excluded.title").run(row.playlist_id,row.channel_id,row.title??"",row.thumbnail??"",row.video_count??""); const existingPolicy=(await database.prepare("SELECT offline_policy FROM user_followed_playlists WHERE user_id=? AND playlist_id=?").get(uid,row.playlist_id) as { offline_policy: string } | null)?.offline_policy; const offlinePolicy=["none","download","keep"].includes(row.offline_policy)?row.offline_policy:(existingPolicy??"none"); await database.prepare("INSERT INTO user_followed_playlists(user_id,playlist_id,followed_at,feed_from,include_in_feed,offline_policy) VALUES(?,?,?,?,?,?) ON CONFLICT(user_id,playlist_id) DO UPDATE SET feed_from=excluded.feed_from,include_in_feed=excluded.include_in_feed,offline_policy=excluded.offline_policy").run(uid,row.playlist_id,row.followed_at,row.feed_from,row.include_in_feed?1:0,offlinePolicy); } }
