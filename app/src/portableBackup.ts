...
async function sectionData(id: string, profile: any | null, referenced: Set<string>): Promise<unknown | unknown[]> {
  ...
  switch (id) {
    ...
    case "profile.subscriptions": return await database.prepare(`SELECT uc.channel_id, uc.followed, uc.playback_speed, uc.caption_mode, uc.caption_language, uc.hide_members_only_from_feed, uc.hide_members_only_on_channel, uc.members_only_visibility, uc.added_at FROM user_channels uc WHERE uc.user_id=?`).all(uid);
    ...
    case "profile.playlists": return Promise.all((await database.prepare("SELECT id, portable_uuid, name, icon, sort_order, created_at FROM user_playlists WHERE user_id=?").all(uid) as any[]).map(async (playlist) => ({ uuid: playlist.portable_uuid, name: playlist.name, icon: playlist.icon, order: playlist.sort_order, createdAt: playlist.created_at, videos: (await database.prepare("SELECT video_id, added_at FROM user_playlist_videos WHERE playlist_id=?").all(playlist.id) as any[]), rules: await database.prepare("SELECT pattern, match_type, field FROM user_playlist_rules WHERE playlist_id=?").all(playlist.id) })));
    case "profile.video-state": return await database.prepare("SELECT video_id, status, bucket, queued_at, show_from, watch_position, watch_duration, watched, liked FROM user_videos WHERE user_id=?").all(uid);
...
export async function commitPortableRestore(adminId: number, id: string, revision: number) {
  ...
  try {
    ...
    const tx = database.transaction(async () => {
      ...
      if (selected.has("instance.plugins")) for (const row of data.get("instance.plugins:") ?? []) { if (!PLUGINS.some((p) => p.id === row.id)) { counts.warnings.push(`Plugin ${row.id} is unavailable`); continue; } await setPluginEnabled(row.id, Boolean(row.enabled)); const adapter=PLUGIN_BACKUP_ADAPTERS.find((item)=>item.id===row.id&&item.scope==="instance"); if(adapter&&row.payload) await adapter.restore(adminId,row.payload); }
      for (const profile of state.manifest.profiles) {
        ...
        if (selected.has("profile.tags")) { if (state.plan!.strategy === "replace") await database.prepare("DELETE FROM tags WHERE user_id=?").run(uid); for (const row of get("profile.tags") ?? []) { let tagId = await mappedObject(state.manifest.sourceInstallationId,"tag",row.uuid) ?? (await database.prepare("SELECT id FROM tags WHERE portable_uuid=? OR (user_id=? AND name=? COLLATE NOCASE) LIMIT 1").get(row.uuid,uid,row.name) as any)?.id; if (!tagId) tagId=Number((await database.prepare("INSERT INTO tags(name,color,filter_only,user_id,portable_uuid) VALUES(?,?,?,?,?) RETURNING id").run(String(row.name),String(row.color||"#7c5cff"),row.filterOnly?1:0,uid,row.uuid)).lastInsertRowid); else await database.prepare("UPDATE tags SET name=?,color=?,filter_only=? WHERE id=? AND user_id=?").run(row.name,row.color,row.filterOnly?1:0,tagId,uid); await saveMapping(state.manifest.sourceInstallationId,"tag",row.uuid,tagId); tagIds.set(row.uuid,tagId); for (const channelId of row.channels??[]) { await ensureChannel({channel_id:channelId}); await database.prepare("INSERT OR IGNORE INTO channel_tags(channel_id,tag_id) VALUES(?,?)").run(channelId,tagId); } for (const videoId of row.videos??[]) if (await database.prepare("SELECT 1 FROM videos WHERE video_id=?").get(videoId)) await database.prepare("INSERT INTO video_tags(video_id,tag_id,source) VALUES(?,?,'manual') ON CONFLICT(video_id,tag_id) DO UPDATE SET source='manual'").run(videoId,tagId); } }
        ...
        if (selected.has("profile.playlists")) { if (state.plan!.strategy === "replace") await database.prepare("DELETE FROM user_playlists WHERE user_id=?").run(uid); for (const row of get("profile.playlists") ?? []) { let playlistId=await mappedObject(state.manifest.sourceInstallationId,"playlist",row.uuid)??(await database.prepare("SELECT id FROM user_playlists WHERE portable_uuid=?").get(row.uuid) as any)?.id; if (!playlistId) playlistId=Number((await database.prepare("INSERT INTO user_playlists(name,icon,sort_order,created_at,user_id,portable_uuid) VALUES(?,?,?,?,?,?) RETURNING id").run(row.name,row.icon??"ListMusic",row.order??0,row.createdAt??new Date().toISOString(),uid,row.uuid)).lastInsertRowid); else await database.prepare("UPDATE user_playlists SET name=?,icon=?,sort_order=? WHERE id=? AND user_id=?").run(row.name,row.icon,row.order,playlistId,uid); await saveMapping(state.manifest.sourceInstallationId,"playlist",row.uuid,playlistId); for (const video of row.videos??[]) if (await database.prepare("SELECT 1 FROM videos WHERE video_id=?").get(video.video_id)) await database.prepare("INSERT OR IGNORE INTO user_playlist_videos(playlist_id,video_id,added_at) VALUES(?,?,?)").run(playlistId,video.video_id,video.added_at??new Date().toISOString()); for (const rule of row.rules??[]) if (!await database.prepare("SELECT 1 FROM user_playlist_rules WHERE playlist_id=? AND lower(pattern)=lower(?) AND match_type=? AND field=?").get(playlistId,rule.pattern,rule.match_type,rule.field)) await database.prepare("INSERT INTO user_playlist_rules(playlist_id,pattern,match_type,field) VALUES(?,?,?,?)").run(playlistId,rule.pattern,rule.match_type,rule.field); } }
        if (selected.has("profile.video-state")) { if (state.plan!.strategy === "replace") await database.prepare("DELETE FROM user_videos WHERE user_id=?").run(uid); for (const row of get("profile.video-state")??[]) if (await database.prepare("SELECT 1 FROM videos WHERE video_id=?").get(row.video_id)) await database.prepare("INSERT INTO user_videos(user_id,video_id,status,bucket,queued_at,show_from,watch_position,watch_duration,watched,liked) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id,video_id) DO UPDATE SET status=excluded.status,bucket=excluded.bucket,queued_at=excluded.queued_at,show_from=excluded.show_from,watch_position=excluded.watch_position,watch_duration=excluded.watch_duration,watched=excluded.watched,liked=excluded.liked").run(uid,row.video_id,row.status??"inbox",row.bucket??null,row.queued_at??null,row.show_from??null,row.watch_position??null,row.watch_duration??null,row.watched??null,row.liked??null); }
